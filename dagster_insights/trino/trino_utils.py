...
# Metadata key prefix used to tag Trino queries with opaque IDs
OPAQUE_ID_METADATA_KEY_PREFIX = "dagster_trino_opaque_id:"
OPAQUE_ID_SQL_SIGIL = "trino_dagster_dbt_v1_opaque_id"

OUTPUT_NON_ASSET_SIGIL = "__trino_query_metadata_"

# Trino cost metadata keys
TRINO_METADATA_EXECUTION_TIME_MS = "__trino_execution_time_ms"
TRINO_METADATA_ROWS_PROCESSED = "__trino_rows_processed"
TRINO_METADATA_BYTES_READ = "__trino_bytes_read"
TRINO_METADATA_QUERY_IDS = "__trino_query_ids"
...
def meter_trino_query(
    ...
    if context.has_assets_def and len(context.assets_def.keys_by_output_name.keys()) == 1:
        ...
        if not associated_asset_key:
            context.log.warn(
                "Op materializes assets, but no asset key associated with Trino query usage."
            )
...
def build_trino_cost_metadata(
    query_ids: Optional[list[str]],
    execution_time_ms: int,
    rows_processed: int = 0,
    bytes_read: int = 0,
) -> dict:
    """Build metadata dictionary for Trino cost information.
    
    Trino costs are typically based on:
    - Query execution time
    - Data read from connectors
    - Cluster compute time
    """
    metadata: dict = {
        TRINO_METADATA_EXECUTION_TIME_MS: execution_time_ms,
        TRINO_METADATA_ROWS_PROCESSED: rows_processed,
        TRINO_METADATA_BYTES_READ: bytes_read,
    }
    if query_ids:
        metadata[TRINO_METADATA_QUERY_IDS] = query_ids
    return metadata

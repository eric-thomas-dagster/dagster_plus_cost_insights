...
def get_current_context_and_asset_key() -> tuple[
    ...
    """Get the current Dagster execution context and associated asset key."""
    asset_key = None
    try:
...
def get_asset_key_for_output(
    ...
    """Get the asset key for a given output name."""
...
def extract_asset_info_from_event(
    ...
    """Extract asset key and partition from a Dagster event."""
...
def handle_raise_on_error(invocation_arg_name: str, arg_position: int = 1):
    """Decorator to handle dbt CLI invocation errors while still emitting cost information."""
    # defer import since dagster_insights shouldn't depend on dagster_dbt

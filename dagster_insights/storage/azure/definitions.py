"""Azure Data Lake Storage cost insights asset and schedule definitions."""
...
from dagster_insights.storage.azure.azure_insights import get_cost_data_for_hour

AZURE_COST_MANAGEMENT_LATENCY_HRS = 24


@dataclass
class AzureInsightsDefinitions:
...
def create_azure_insights_asset_and_schedule(
    start_date: Union[datetime, date, str],
    subscription_id: str,
    name: Optional[str] = None,
    group_name: Optional[str] = None,
    job_name: str = "azure_insights_import",
    dry_run=False,
    allow_partial_partitions=True,
    azure_resource_key: str = "azure",
    cost_management_latency_hrs: int = AZURE_COST_MANAGEMENT_LATENCY_HRS,
...
) -> AzureInsightsDefinitions:
    """Generates a pre-defined Dagster asset and schedule for importing Azure Data Lake Storage cost data.

    Args:
        start_date: The date to start the partitioned schedule on.
        subscription_id: Azure subscription ID.
        name: The name of the asset. Defaults to "azure_cost_history".
        group_name: The name of the asset group.
        job_name: The name of the job. Defaults to "azure_insights_import".
        ...
        azure_resource_key: The name of the Azure resource key. Defaults to "azure".
        cost_management_latency_hrs: Hours to wait before querying Cost Management API. Defaults to 24.
    ...
    @asset(
        name=name or "azure_cost_history",
        group_name=group_name,
        partitions_def=partitions_def,
        required_resource_keys={azure_resource_key},
        io_manager_def=fs_io_manager,
    )
    def poll_azure_cost_history(
        ...
        try:
            ...
            raise ImportError(
                "azure-mgmt-costmanagement is required. "
                "Install it with: pip install azure-mgmt-costmanagement"
            )

        azure_resource = getattr(context.resources, azure_resource_key)
        try:
            cost_management_client = azure_resource.get_cost_management_client()
        except AttributeError:
            # Fallback: create client directly (requires credentials)
        ...
        earliest_call_time = end_time + timedelta(hours=cost_management_latency_hrs)
        if now < earliest_call_time:
            err = (
                f"Attempted to gather Azure cost information before Cost Management data may be "
        ...
        context.log.info(
            f"Fetched Azure cost information from {start_time.isoformat()} to {end_time.isoformat()}"
        )
        ...
        if dry_run:
            ...
            context.log.info(
                f"Submitting cost information for {len(costs)} Azure resources to Dagster Insights"
            )
            put_cost_information(
                context=context,
                metric_name="azure_storage_cost_usd",
    ...
    insights_job = define_asset_job(
        job_name,
        AssetSelection.assets(poll_azure_cost_history),
        partitions_def=partitions_def,
    )
    ...
    return AzureInsightsDefinitions(
        assets=[poll_azure_cost_history],

...
# Metadata key prefix used to tag Azure SQL queries with opaque IDs
OPAQUE_ID_METADATA_KEY_PREFIX = "dagster_azuresql_opaque_id:"
OPAQUE_ID_SQL_SIGIL = "azuresql_dagster_dbt_v1_opaque_id"

OUTPUT_NON_ASSET_SIGIL = "__azuresql_query_metadata_"

# Azure SQL cost metadata keys
AZURESQL_METADATA_DTU_SECONDS = "__azuresql_dtu_seconds"
AZURESQL_METADATA_EXECUTION_TIME_MS = "__azuresql_execution_time_ms"
AZURESQL_METADATA_QUERY_IDS = "__azuresql_query_ids"
AZURESQL_METADATA_ROWS_PROCESSED = "__azuresql_rows_processed"
...
def meter_azuresql_query(
    ...
    if context.has_assets_def and len(context.assets_def.keys_by_output_name.keys()) == 1:
        ...
        if not associated_asset_key:
            context.log.warn(
                "Op materializes assets, but no asset key associated with Azure SQL query usage."
            )
...
def build_azuresql_cost_metadata(
    query_ids: Optional[list[str]],
    dtu_seconds: float,
    execution_time_ms: int,
    rows_processed: int = 0,
) -> dict:
    """Build metadata dictionary for Azure SQL Database cost information.
    
    DTU (Database Transaction Units) is the compute unit for Azure SQL Database.
    """
    metadata: dict = {
        AZURESQL_METADATA_DTU_SECONDS: dtu_seconds,
        AZURESQL_METADATA_EXECUTION_TIME_MS: execution_time_ms,
        AZURESQL_METADATA_ROWS_PROCESSED: rows_processed,
    }
    if query_ids:
        metadata[AZURESQL_METADATA_QUERY_IDS] = query_ids
    return metadata

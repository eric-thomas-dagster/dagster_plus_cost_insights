...
def dbt_with_azuresql_insights(
    ...
    """Wraps a dagster-dbt invocation to associate each Azure SQL query with the produced
    ...
    Example:
        .. code-block:: python
            ...
            from dagster_insights import dbt_with_azuresql_insights

            @dbt_assets(manifest=DBT_MANIFEST_PATH)
            def my_dbt_assets(context: AssetExecutionContext, dbt: DbtCliResource):
                dbt_cli_invocation = dbt.cli(["build"], context=context)
                yield from dbt_with_azuresql_insights(context, dbt_cli_invocation)
    """
    if not skip_config_check:
        adapter_type = dbt_cli_invocation.manifest["metadata"]["adapter_type"]
        # Azure SQL can use either sqlserver or azuresql adapter
        if adapter_type not in ("sqlserver", "azuresql"):
            raise RuntimeError(
                f"The 'sqlserver' or 'azuresql' adapter must be used but instead found '{adapter_type}'"
            )

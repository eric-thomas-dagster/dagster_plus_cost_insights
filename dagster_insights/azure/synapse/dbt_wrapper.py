...
def dbt_with_synapse_insights(
    ...
    """Wraps a dagster-dbt invocation to associate each Azure Synapse query with the produced
    ...
    **Example:**
    ...
    if not skip_config_check:
        ...
        if adapter_type not in ["synapse", "sqlserver"]:
            raise RuntimeError(
                f"The 'synapse' or 'sqlserver' adapter must be used but instead found '{adapter_type}'"
            )

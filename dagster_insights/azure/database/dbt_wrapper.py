...
def dbt_with_azure_database_insights(
    ...
    """Wraps a dagster-dbt invocation to associate each Azure Database query with the produced
    ...
    Example:
        .. code-block:: python
            ...
            from dagster_insights import dbt_with_azure_database_insights

            @dbt_assets(manifest=DBT_MANIFEST_PATH)
            def my_dbt_assets(context: AssetExecutionContext, dbt: DbtCliResource):
                dbt_cli_invocation = dbt.cli(["build"], context=context)
                yield from dbt_with_azure_database_insights(context, dbt_cli_invocation)
    """
    if not skip_config_check:
        adapter_type = dbt_cli_invocation.manifest["metadata"]["adapter_type"]
        # Azure Database supports both PostgreSQL and MySQL
